...
contract CryptoABS is StandardToken, Ownable {
  ...
  uint256 public minInvestInWei;                        // ?????? in wei
  uint256 public tokenExchangeRateInWei;                // 1 Token = n ETH in wei

  uint256 public startBlock;                            // ICO ??? block number
  uint256 public endBlock;                              // ICO ??? block number
  uint256 public maxTokenSupply;                        // ICO ? max token??? USD to ETH ????
  
  uint256 public initializedTime;                       // ???????????????
  uint256 public financingPeriod;                       // token ????
  uint256 public tokenLockoutPeriod;                    // token ?????????? transfer
  uint256 public tokenMaturityPeriod;                   // token ???

  bool public paused;                                   // ????????
  bool public initialized;                              // ????
  uint256 public finalizedBlock;                        // ???????????
  uint256 public finalizedTime;                         // ?????????
  uint256 public finalizedCapital;                      // ????? ETH ??

  struct ExchangeRate {
    uint256 blockNumber;                                // block number
    uint256 exchangeRateInWei;                          // 1 USD = n ETH in wei, ???????????
  }
