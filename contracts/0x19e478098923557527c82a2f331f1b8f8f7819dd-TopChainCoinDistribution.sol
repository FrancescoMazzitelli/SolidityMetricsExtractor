...
contract TopChainCoinDistribution is Ownable, Authorizable {
    ...
    uint totalToken = 2100000000 * (10 ** 6); //21?

    uint public privateTokenCap = 210000000 * (10 ** 6); //????2.1?(10%)

    uint public marketToken = 315000000 * (10 ** 6); //????3.15?(15%)

    uint public operationToken = 210000000 * (10 ** 6); //????2.1?(10%)

    uint public gameMiningTokenCap = 1155000000 * (10 ** 6); //????11.55?(55%)

    uint public teamToken2018 = 105000000 * (10 ** 6); //????2.1?(10%),2018???1.05?(5%)
    uint public teamToken2019 = 105000000 * (10 ** 6); //????2.1?(10%),2019???1.05?(5%)
    ...
    uint public firstYearGameMiningTokenCap = 577500000 * (10 ** 6); //2018?5.775?(21?*0.55*0.5)??????? 
